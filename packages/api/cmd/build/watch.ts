...
const opts = program
  .option('-m', 'Create a migration file that corresponds to the differences between the database and schema.')
  .option('-r', 'Reset the database and seed it by executing the corresponding script.')
...
(async function main() {
  ...
  await (opts.m ? spawn('npm', ['exec', '-c', 'prisma migrate dev']) : spawn('npm', ['exec', '-c', 'prisma db push']));
  ...
  if (opts.s) {
    void spawn('npm', ['exec', '-c', 'prisma studio --browser none --port 1234']);
  }

  await spawn('npm', ['exec', '-c', 'ts-node-dev --inspect --transpile-only --exit-child -- ./src/index.ts']);
})();
