 ...
 */
export const RoomList: FC<IProps> = ({ selectedRoomId, viewerId, rooms, onRoomSelect }) => {
  ...
  return (
    <List>
      <Stack divider={<Divider />}>
        {viewerRoom && (
          <ListItemButton onClick={() => onRoomSelect(viewerRoom.id)} selected={viewerRoom.id === selectedRoomId}>
            <ListItemText primary={viewerRoom.name || 'Saved messages'} />
          </ListItemButton>
        )}

        {rest.map(room => (
          <ListItemButton key={room.id} onClick={() => onRoomSelect(room.id)} selected={room.id === selectedRoomId}>
            <ListItemText
              primary={
                room.name ||
                room.members
